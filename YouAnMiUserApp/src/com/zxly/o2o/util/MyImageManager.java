...
public class MyImageManager {
	...
	private static MyImageManager imageManager;
...
//	private static MyApplication myapp;
	 ...
	 * @return
	 */
	public static MyImageManager from(Context context) {
		...
		if (imageManager == null) {
			imageManager = new MyImageManager(AppController.getInstance());
		}
	 ...
	 * @param context
	 */
	private MyImageManager(Context context) {
	...
	public void displayImage(ImageView imageView, String url, int resId,
		...
		imageView.setTag(url);
		// 读取map缓存
		BasicMyCircleAct.photoBitmap = mMemoryCache.get(url + width + height);
		if (BasicMyCircleAct.photoBitmap != null) {
			setImageBitmap(imageView, BasicMyCircleAct.photoBitmap, false);
			return;
		}
...
//				Bitmap bitmap = null;
				...
				if (msg.obj != null && msg.obj instanceof ImageRef) {
					...
					if (url.toLowerCase().contains(".")) {
						...
						if (imageRef.width != 0 && imageRef.height != 0) {
							BasicMyCircleAct.photoBitmap = ThumbnailUtils.extractThumbnail(tBitmap,
									imageRef.width, imageRef.height,
									ThumbnailUtils.OPTIONS_RECYCLE_INPUT);
							isFromNet = true;
						} else {
							BasicMyCircleAct.photoBitmap = tBitmap;
							tBitmap = null;
						}

					} else{
						BasicMyCircleAct.photoBitmap = mDiskCache.get(url);
					}
					...
					if (BasicMyCircleAct.photoBitmap != null) {
						...
						if (imageRef.width != 0 && imageRef.height != 0) {
							if (mMemoryCache.get(url + imageRef.width
									+ imageRef.height) == null&&BasicMyCircleAct.photoBitmap!=null)
								mMemoryCache.put(url + imageRef.width
										+ imageRef.height, BasicMyCircleAct.photoBitmap);
						} else {
							if (mMemoryCache.get(url) == null&&BasicMyCircleAct.photoBitmap!=null)
								mMemoryCache.put(url, BasicMyCircleAct.photoBitmap);
						}
				...
				if (mImageManagerHandler != null) {
					Message message = mImageManagerHandler.obtainMessage(
							MSG_REPLY, BasicMyCircleAct.photoBitmap);
	...
	public String urlToFilePath(String url) {
		...
		// 图片存取路径
		filePath.append(AppController.getInstance().getCacheDir().toString()).append('/');

		// 图片文件名 
		filePath.append(MD5Util.GetMD5Code(url)).append(url.substring(index));
