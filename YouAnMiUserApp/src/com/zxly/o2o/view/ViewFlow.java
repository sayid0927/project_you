...
public class ViewFlow extends AdapterView<Adapter> {
	...
	private float mPositionX=0,mPositionY=0;
	...
	public void continueAutoFlowTimer(){
		if(handler!=null && isAnimalOn){
	...
	public void pauseAutoFlowTimer(){
		if(handler!=null)
			handler.removeMessages(0);
	}
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			if(null != mViewTouchListener)
				mViewTouchListener.setTouchState(false);
			/*
		...
		case MotionEvent.ACTION_MOVE:
			if((Math.abs(ev.getY()-mPositionY)>2)||(Math.abs(ev.getX()-mPositionX)>2)){
				if (!mScroller.isFinished()) {
					mScroller.abortAnimation();
				}
		    }
		...
		case MotionEvent.ACTION_UP:
			if(null != mViewTouchListener)
				mViewTouchListener.setTouchState(true);
			if (mTouchState == TOUCH_STATE_SCROLLING) {
...
//			}
	...
	public static interface ViewTouchListener {
		void setTouchState(boolean isTouched);
	}
	
	public void setOnViewTouchListener(ViewTouchListener l) {
		mViewTouchListener = l;
	}
