...
import com.zxly.o2o.view.ViewFlow.FlowIndicator;
...
//		fadeOutTime = a.getInt(R.styleable.CircleFlowIndicator_fadeOut, 0);
	...
	protected void onDraw(Canvas canvas) {
		...
		Drawable off = getResources().getDrawable(R.drawable.banner_off);
		Drawable on = getResources().getDrawable(R.drawable.banner_on);
		
		// Draw stroked circles
		for (int iLoop = 0; iLoop < count; iLoop++) {
			canvas.drawBitmap(((BitmapDrawable)off).getBitmap(),leftPadding + radius
					+ (iLoop * circleSeparation) + centeringOffset,
					getPaddingTop(), mPaintInactive);
		}
		float cx = 0;
		if (flowWidth != 0) {
		...
		float offset = leftPadding + radius + cx+centeringOffset;
		canvas.drawBitmap(((BitmapDrawable)on).getBitmap(),offset <= maxWidth-14 ? offset : 9, getPaddingTop()
				, mPaintActive);
	}
	...
	private int measureWidth(int measureSpec) {
		...
		// Calculate the width according the views count
			...
			result = (int) (getPaddingLeft() + getPaddingRight()
					+ (count * 2 * radius) + (count - 1) * temp + 1)+4;
