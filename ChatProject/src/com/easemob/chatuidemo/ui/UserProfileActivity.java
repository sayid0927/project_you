...
public class UserProfileActivity extends EaseBaseActivity implements OnClickListener{
	...
	public void asyncFetchUserInfo(String username){
		HXHelper.getInstance().getUserProfileManager().asyncGetUserInfo(username, new EMValueCallBack<EaseYAMUser>() {
			
			@Override
			public void onSuccess(EaseYAMUser user) {
				if (user != null) {
					tvNickName.setText(user.getFirendsUserInfo().getNickname());
					if(!TextUtils.isEmpty(user.getFirendsUserInfo().getAvatar())){
						 Picasso.with(UserProfileActivity.this).load(user.getFirendsUserInfo().getAvatar()).placeholder(R.drawable.em_default_avatar).into(headAvatar);
					}else{
						Picasso.with(UserProfileActivity.this).load(R.drawable.em_default_avatar).into(headAvatar);
					}
					HXHelper.getInstance().saveYAMContact(user);
				}
			}
