...
public class ChatFragment extends EaseChatFragment implements EaseChatFragmentListener {
    ...
    private static final int ITEM_PRODUCE = 15;
    private static final int ITEM_ARTICLE = 16;
    private static final int ITEM_ACTIVITY = 17;
...
//        }
    ...
    protected void registerExtendMenuItem() {
        ...
        //增加扩展item 商户端需要添加三个拓展栏
        if (chatType == HXConstant.CHATTYPE_SINGLE && EaseConstant.shopID < 0) {
            inputMenu
                    .registerExtendMenuItem(R.string.push_article, R.drawable.chat_article_selector,
                            ITEM_ARTICLE,
                            extendMenuItemClickListener);
            inputMenu.registerExtendMenuItem(R.string.push_produce, R.drawable.chat_shop_selector,
                    ITEM_PRODUCE,
                    extendMenuItemClickListener);
            inputMenu.registerExtendMenuItem(R.string.push_activity, R.drawable.chat_activity_selector,
                    ITEM_ACTIVITY, extendMenuItemClickListener);
    ...
    public void onEnterToChatDetails() {
        if (chatType == HXConstant.CHATTYPE_GROUP) {
    ...
    public void onAvatarClick(String username) {
        ...
        if (EaseConstant.shopID > 0) {
            EaseConstant.startIMUserDetailInfo(HXApplication.getInstance().parseIdFromName(username), false,
                    getActivity(), "个人信息", 2, username.substring(username.lastIndexOf("_")));
        } else if (EaseConstant.shopID < 0) {
            if (username.contains("_") && username.indexOf("_") != username.lastIndexOf("_")) {
                EaseConstant.startShopIMUserDetailInfo(HXApplication.getInstance().parseIdFromName(username),
                        getActivity());
            }
        }
    ...
    public void onMessageBubbleLongClick(EMMessage message) {
        //消息框长按
        startActivityForResult(
                (new Intent(getActivity(), ContextMenuActivity.class)).putExtra("message", message),
    ...
    public boolean onExtendMenuItemClick(int itemId, View view) {
        switch (itemId) {
            ...
            case ITEM_PRODUCE: //商品软文
                if (EaseConstant.shopID < 0) {  //已登录
                    EaseConstant.users.add(toChatUsername);
                    EaseConstant.startEaseMultiPushActivity(getActivity(),
                            -HXConstant.PUSH_SHOP_INFO, HXConstant.MUL_SEND);
                    onBackPressed();
                }
                break;

            case ITEM_ARTICLE: //文章软文
                if (EaseConstant.shopID < 0) {  //已登录
                    EaseConstant.users.add(toChatUsername);
                    EaseConstant.startEaseMultiPushActivity(getActivity(),
                            -HXConstant.PUSH_ARTICLE, HXConstant.MUL_SEND);
                }
                break;

            case ITEM_ACTIVITY: //活动软文
                if (EaseConstant.shopID < 0) {  //已登录
                    EaseConstant.users.add(toChatUsername);
                    EaseConstant.startEaseMultiPushActivity(getActivity(),
                            -HXConstant.PUSH_CAMPAIGN, HXConstant.MUL_SEND);
                }
                break;
    ...
    protected void startVideoCall() {
        if (!EMChatManager.getInstance().isConnected()) {
            ...
            startActivity(
                    new Intent(getActivity(), VideoCallActivity.class).putExtra("username", toChatUsername)
                            .putExtra("isComingCall", false));
