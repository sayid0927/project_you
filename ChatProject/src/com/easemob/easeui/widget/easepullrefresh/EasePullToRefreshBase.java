...
public abstract class EasePullToRefreshBase<T extends View> extends LinearLayout implements
		EaseIPullToRefresh<T> {
	...
	private EaseLoadingLayout mHeaderLayout;

	private EaseLoadingLayout mFooterLayout;
	...
	public EasePullToRefreshBase(Context context) {
		super(context);
		init(context, null);
	}

	public EasePullToRefreshBase(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context, attrs);
	}

	public EasePullToRefreshBase(Context context, Mode mode) {
		super(context);
		mMode = mode;
		init(context, null);
	}

	public EasePullToRefreshBase(Context context, Mode mode, AnimationStyle animStyle) {
		super(context);
		mMode = mode;
	...
	@Override
	public final EaseILoadingLayout getLoadingLayoutProxy() {
		return getLoadingLayoutProxy(true, true);
	}

	@Override
	public final EaseILoadingLayout getLoadingLayoutProxy(boolean includeStart, boolean includeEnd) {
	...
	public final boolean isPullToRefreshOverScrollEnabled() {
		return VERSION.SDK_INT >= VERSION_CODES.GINGERBREAD && mOverScrollEnabled
				&& EaseOverscrollHelper.isAndroidOverScrollEnabled(mRefreshableView);
	}
	...
	protected EaseLoadingLayout createLoadingLayout(Context context, Mode mode, TypedArray attrs) {
		EaseLoadingLayout layout = mLoadingAnimationStyle.createLoadingLayout(context, mode,
	 ...
	 */
	protected EaseLoadingLayoutProxy createLoadingLayoutProxy(final boolean includeStart,
			final boolean includeEnd) {
		EaseLoadingLayoutProxy proxy = new EaseLoadingLayoutProxy();
	...
	protected final EaseLoadingLayout getFooterLayout() {
	...
	protected final EaseLoadingLayout getHeaderLayout() {
	...
	protected final void setHeaderScroll(int value) {
		...
		if (USE_HW_LAYERS) {
			 ...
			 */
			EaseViewCompat.setLayerType(mRefreshableViewWrapper, value != 0 ? View.LAYER_TYPE_HARDWARE
	...
	private void init(Context context, AttributeSet attrs) {
		...
		// Styleables from XML
		TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.EasePullToRefresh);

		if (a.hasValue(R.styleable.EasePullToRefresh_easeptrMode)) {
			mMode = Mode.mapIntToValue(a.getInteger(R.styleable.EasePullToRefresh_easeptrMode, 0));
		}

		if (a.hasValue(R.styleable.EasePullToRefresh_easeptrAnimationStyle)) {
			mLoadingAnimationStyle = AnimationStyle.mapIntToValue(a.getInteger(
					R.styleable.EasePullToRefresh_easeptrAnimationStyle, 0));
		}
		 ...
		 * Styleables from XML
		 */
		if (a.hasValue(R.styleable.EasePullToRefresh_easeptrRefreshableViewBackground)) {
			Drawable background = a
					.getDrawable(R.styleable.EasePullToRefresh_easeptrRefreshableViewBackground);
			if (null != background) {
				mRefreshableView.setBackgroundDrawable(background);
			}
		} else if (a.hasValue(R.styleable.EasePullToRefresh_easeptrAdapterViewBackground)) {
			EasePullUtils.warnDeprecation("ptrAdapterViewBackground", "ptrRefreshableViewBackground");
			Drawable background = a.getDrawable(R.styleable.EasePullToRefresh_easeptrAdapterViewBackground);
			if (null != background) {
				mRefreshableView.setBackgroundDrawable(background);
			}
		}

		if (a.hasValue(R.styleable.EasePullToRefresh_easeptrOverScroll)) {
			mOverScrollEnabled = a.getBoolean(R.styleable.EasePullToRefresh_easeptrOverScroll, true);
		}

		if (a.hasValue(R.styleable.EasePullToRefresh_easeptrScrollingWhileRefreshingEnabled)) {
			mScrollingWhileRefreshingEnabled = a.getBoolean(
					R.styleable.EasePullToRefresh_easeptrScrollingWhileRefreshingEnabled, false);
		}
	...
	public static enum AnimationStyle {
		...
		EaseLoadingLayout createLoadingLayout(Context context, Mode mode, Orientation scrollDirection,
				TypedArray attrs) {
			switch (this) {
			case ROTATE:
			default:
				return new EaseRotateLoadingLayout(context, mode, scrollDirection, attrs);
			case FLIP:
				return new EaseFlipLoadingLayout(context, mode, scrollDirection, attrs);
			}
		}
	}
	...
	public static enum Mode {
		 ...
		 * {@link EasePullToRefreshBase#setRefreshing() setRefreshing()}.
		 */
	 ...
	 * {@link EasePullToRefreshAdapterViewBase#setOnLastItemVisibleListener}.
	 * 
	 * @author Chris Banes
	 */
	 ...
	 * (such as sounds). See (
	 * {@link EasePullToRefreshAdapterViewBase#setOnPullEventListener}.
	 * 
	 * @author Chris Banes
	 */
	public static interface OnPullEventListener<V extends View> {
		 ...
		 */
		public void onPullEvent(final EasePullToRefreshBase<V> refreshView, State state, Mode direction);
	...
	public static interface OnRefreshListener<V extends View> {
		 ...
		 * end
		 */
		public void onRefresh(final EasePullToRefreshBase<V> refreshView);
	...
	public static interface OnRefreshListener2<V extends View> {
		 ...
		 */
		public void onPullDownToRefresh(final EasePullToRefreshBase<V> refreshView);

		/**
		 * onPullUpToRefresh will be called only when the user has Pulled from
		 * the end, and released.
		 */
		public void onPullUpToRefresh(final EasePullToRefreshBase<V> refreshView);
	...
	final class SmoothScrollRunnable implements Runnable {
		...
		public void run() {
			...
			if (mContinueRunning && mScrollToY != mCurrentY) {
				EaseViewCompat.postOnAnimation(EasePullToRefreshBase.this, this);
			} else {
				if (null != mListener) {
