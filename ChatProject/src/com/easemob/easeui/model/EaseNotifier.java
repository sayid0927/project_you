...
import com.easemob.chatuidemo.task.EaseNormalDownloadImageTask;
...
public class EaseNotifier {
    ...
    public synchronized void onNewTuWenMsg(EMMessage message, boolean isBackground, String objectString) {
        if (EMChatManager.getInstance().isSlientMessage(message)) {
            return;
        }
        EaseSettingsProvider settingsProvider = EaseUI.getInstance().getSettingsProvider();
        ...
        //        sendNotification(message, false);
        sendTuWenNotification(message, isBackground, objectString);

        viberateAndPlayTone(message);
    }


    public synchronized void sendTuWenNotification(EMMessage message, final boolean isForeground,
                                                   String objectString) {
        final EaseTuWenVO easeTuWenVO;
        try {

            easeTuWenVO = GsonParser.getInstance().fromJson(objectString, new TypeToken<EaseTuWenVO>() {
            });
            //            message.getStringAttribute(HXConstant.SYS_TUWEN_PUSH_TYPE);
            ...
            msgIntent = setIntent(message, easeTuWenVO, msgIntent);

            PendingIntent pendingIntent = PendingIntent
                    .getActivity(appContext, what, msgIntent, PendingIntent.FLAG_UPDATE_CURRENT);
            ...
            final RemoteViews remoteView = new RemoteViews(HXApplication.applicationContext.getPackageName(),
                    R.layout.ease_row_tuwen_for_push);

            new EaseNormalDownloadImageTask(new EaseNormalDownloadImageTask.DownloadFileCallback() {
                @Override
                public void afterDownload(Bitmap bitmap) {
                    remoteView.setImageViewBitmap(R.id.iv_icon, bitmap);

                }
            }).execute(easeTuWenVO.getUrl());

            remoteView.setTextViewText(R.id.title,
                    easeTuWenVO.getTitle());
            remoteView.setTextViewText(R.id.title_below_left,
                    easeTuWenVO.getContent());
            notification.contentView = remoteView;

            if (isForeground) {
                notificationManager.notify(foregroundNotifyID,
                        notification);
                notificationManager
                        .cancel(foregroundNotifyID);
            } else {
                notificationManager
                        //                        .notify(notifyID, notification);
                        .notify(what, notification);
            }
    ...
    private Intent setIntent(EMMessage message, EaseTuWenVO easeTuWenVO, Intent intent) {
        try {
            int what = message.getIntAttribute("what");

            switch (Math.abs(what)) {
                case HXConstant.PUSH_ARTICLE:
                    intent.setClass(appContext, EaseH5DetailAct.class);
                    intent.putExtra("sys_msg_url", easeTuWenVO.getH5Url());
                    intent.putExtra("title", "文章详情");
                    break;

                case HXConstant.PUSH_CAMPAIGN:
                    intent.setClass(appContext, EaseH5DetailAct.class);
                    if (easeTuWenVO.getH5Url() != null) {
                        intent.putExtra("sys_msg_url",
                ...
                case HXConstant.PUSH_SHOP_INFO:
                     intent = new Intent("com.zxly.o2o.activity.ProductInfoAct");
                    intent.putExtra("bannerId", -1);
                    intent.putExtra("produceId", easeTuWenVO.getId());  //商品id
                    break;
                default:   //后台推送
                    if (easeTuWenVO.getUrl() == null) {
                        intent.setClass(appContext, SysMsgDetailActivity.class);
                        intent.putExtra("title", easeTuWenVO.getTitle());
                        intent.putExtra("content", easeTuWenVO.getContent());
                        intent.putExtra("time", easeTuWenVO.getCreateTime());
                        if (what == 100) {  //门店快讯
                            intent.putExtra("id", easeTuWenVO.getId());
                        }
                    } else {
                        easeTuWenVO.setUrl(easeTuWenVO.getUrl().replace("{userId}", "0"));
                        intent.setClass(appContext, EaseH5DetailAct.class);
                        intent.putExtra("sys_msg_url", easeTuWenVO.getH5Url());
                        intent.putExtra("title", easeTuWenVO.getTitle());
                        intent.putExtra("id", easeTuWenVO.getId());
...
//        }
    ...
    protected void sendNotification(EMMessage message, boolean isForeground, boolean numIncrease) {
        ...
        try {
            ...
            notifyText=setNotiContent(message,ticker);


            //修改标题,这里使用默认
            try {
                contentTitle= message.getStringAttribute("nickname");
            } catch (EaseMobException e) {
                e.printStackTrace();
            }
...
//            }
    ...
    private String setNotiContent(EMMessage message,String ticker) {
